...
WITH tpl AS (
    ...
    WHERE
        CASE
            ...
            ELSE $8 != 'visual' AND is_default = 1
        END
    LIMIT 1
),
camp AS (
...
med AS (
    INSERT INTO campaign_media (campaign_id, media_id, filename)
        (SELECT (SELECT id FROM camp), id, filename FROM media WHERE id IN (SELECT value FROM json_each(listmonk_array($20))))
),
insLists AS (
    INSERT INTO campaign_lists (campaign_id, list_id, list_name)
        SELECT (SELECT id FROM camp), id, name FROM lists WHERE id IN (SELECT value FROM json_each(listmonk_array($15)))
)
SELECT id FROM camp;
...
SELECT  c.*,
        COUNT(*) OVER () AS total,
        COALESCE((SELECT json_group_array(json_object('id',COALESCE(campaign_lists.list_id,0),'name',campaign_lists.list_name))
          FROM campaign_lists WHERE campaign_lists.campaign_id=c.id),'[]') AS lists
FROM campaigns c
WHERE ($1 = 0 OR id = $1)
    AND (json_array_length(listmonk_array($2)) = 0 OR status IN (SELECT value FROM json_each(listmonk_array($2))))
    AND NOT EXISTS (SELECT 1 FROM json_each(listmonk_array($3)) requested WHERE requested.value NOT IN (SELECT value FROM json_each(listmonk_array(c.tags))))
    AND ($4 = '' OR (c.name || ' ' || c.subject) LIKE $4)
    -- Get all campaigns or filter by list IDs.
    AND (
        $5 OR EXISTS (
            SELECT 1 FROM campaign_lists WHERE campaign_id = c.id AND list_id IN (SELECT value FROM json_each(listmonk_array($6)))
        )
    )
ORDER BY %order% LIMIT (CASE WHEN $8 < 1 THEN -1 ELSE $8 END) OFFSET $7;
...
SELECT COUNT(*) OVER () AS total, campaigns.*,
    ...
    WHERE campaigns.archive=1 AND campaigns.type='regular' AND campaigns.status IN ('running','paused','finished')
    ORDER by campaigns.created_at DESC LIMIT $2 OFFSET $1;
...
WITH lists AS (
    SELECT campaign_id, json_group_array(json_object('id',list_id,'name',list_name)) AS lists FROM campaign_lists
    WHERE campaign_id IN (SELECT value FROM json_each(listmonk_array($1))) GROUP BY campaign_id
),
media AS (
    SELECT campaign_id, json_group_array(json_object('id',media_id,'filename',filename)) AS media FROM campaign_media
    WHERE campaign_id IN (SELECT value FROM json_each(listmonk_array($1))) GROUP BY campaign_id
),
views AS (
    SELECT campaign_id, COUNT(campaign_id) as num FROM campaign_views
    WHERE campaign_id IN (SELECT value FROM json_each(listmonk_array($1)))
...
LEFT JOIN bnc AS b ON (b.campaign_id = id)
ORDER BY x.key;

-- name: get-campaign-for-preview
SELECT campaigns.*, COALESCE(templates.body, '') AS template_body,
COALESCE((SELECT json_group_array(json_object('id',COALESCE(campaign_lists.list_id,0),'name',campaign_lists.list_name))
 FROM campaign_lists WHERE campaign_lists.campaign_id=campaigns.id),'[]') AS lists
FROM campaigns
...
-- Returns 1 if the campaign $1 has any of the lists given in $2.
SELECT EXISTS (
    SELECT 1 FROM campaign_lists WHERE campaign_id = $1 AND list_id IN (SELECT value FROM json_each(listmonk_array($2)))
);
...
WITH camps AS (
    ...
    SELECT campaigns.*, COALESCE(templates.body, (SELECT body FROM templates WHERE is_default = 1 LIMIT 1), '') AS template_body
    FROM campaigns
    LEFT JOIN templates ON (templates.id = campaigns.template_id)
    WHERE (status='running' OR (status='scheduled' AND CURRENT_TIMESTAMP >= campaigns.send_at))
    AND NOT(campaigns.id IN (SELECT value FROM json_each(listmonk_array($1))))
),
campLists AS (
...
u AS (
    ...
    SET to_send = co.to_send,
        ...
        started_at=(CASE WHEN ca.started_at IS NULL THEN CURRENT_TIMESTAMP ELSE ca.started_at END)
...
WITH uniqIDs AS (
    SELECT DISTINCT subscriber_id,campaign_id,CASE WHEN julianday($3)-julianday($2)>=7 THEN date(created_at) ELSE strftime('%%Y-%%m-%%d %%H:00:00',created_at) END AS "timestamp"
    FROM %s
    WHERE campaign_id IN (SELECT value FROM json_each(listmonk_array($1))) AND created_at >= $2 AND created_at <= $3
...
-- raw: 1
SELECT campaign_id,COUNT(*) AS "count",CASE WHEN julianday($3)-julianday($2)>=7 THEN date(created_at) ELSE strftime('%%Y-%%m-%%d %%H:00:00',created_at) END AS "timestamp"
    FROM %s
    WHERE campaign_id IN (SELECT value FROM json_each(listmonk_array($1))) AND created_at >= $2 AND created_at <= $3
...
SELECT COUNT(%s) AS "count", url
    ...
    WHERE campaign_id IN (SELECT value FROM json_each(listmonk_array($1))) AND link_clicks.created_at >= $2 AND link_clicks.created_at <= $3
    GROUP BY links.url ORDER BY "count" DESC LIMIT 50;

-- name: export-campaign-views
SELECT campaign_views.campaign_id,
       COALESCE(campaigns.uuid, '') AS campaign_uuid,
       COALESCE(campaigns.name, '') AS campaign_name,
       COALESCE(campaign_views.subscriber_id, 0) AS subscriber_id,
       COALESCE(subscribers.uuid, '') AS subscriber_uuid,
...
subs AS (
    ...
    FROM (
        ...
        WHERE
            sl.list_id IN (SELECT value FROM json_each(listmonk_array($5)))
            -- last_subscriber_id
            AND s.id > $3
             -- max_subscriber_id
            AND s.id <= $4
...
u AS (
    UPDATE campaigns
    SET last_subscriber_id = (SELECT MAX(id) FROM subs), updated_at = CURRENT_TIMESTAMP
...
WHERE subscriber_lists.list_id IN (SELECT list_id FROM campaign_lists where campaign_id=$1 AND list_id IS NOT NULL)
...
clists AS (
    ...
    DELETE FROM campaign_lists WHERE campaign_id = $1 AND NOT(list_id IN (SELECT value FROM json_each(listmonk_array($14))))
),
med AS (
    DELETE FROM campaign_media WHERE campaign_id = $1
    AND ( media_id IS NULL or NOT(media_id IN (SELECT value FROM json_each(listmonk_array($19))))) RETURNING media_id
),
medi AS (
    INSERT INTO campaign_media (campaign_id, media_id, filename)
        (SELECT $1 AS campaign_id, id, filename FROM media WHERE id IN (SELECT value FROM json_each(listmonk_array($19))))
        ON CONFLICT (campaign_id, media_id) DO NOTHING
)
INSERT INTO campaign_lists (campaign_id, list_id, list_name)
    (SELECT $1 as campaign_id, id, name FROM lists WHERE id IN (SELECT value FROM json_each(listmonk_array($14))))
...
WHERE (
    CASE
        WHEN json_array_length(listmonk_array($1)) > 0 THEN id IN (SELECT value FROM json_each(listmonk_array($1)))
        ELSE $2 = '' OR (name || ' ' || subject) LIKE $2
    END
)
-- Get all campaigns or filter by permitted list IDs.
AND (
    $3 OR EXISTS (
        SELECT 1 FROM campaign_lists WHERE campaign_id = campaigns.id AND list_id IN (SELECT value FROM json_each(listmonk_array($4)))
    )
);
...
WITH view AS (
    ...
    LEFT JOIN subscribers ON (CASE WHEN $2 != '' THEN subscribers.uuid = $2 ELSE 0 END)
